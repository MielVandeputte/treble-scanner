...
export function ContextProvider({ children }: Readonly<{ children: ReactNode }>) {
  ...
  useEffect(() => {
    globalThis.addEventListener('online', refreshInternetConnectedState);
    globalThis.addEventListener('offline', refreshInternetConnectedState);
    ...
    return () => {
      globalThis.removeEventListener('online', refreshInternetConnectedState);
      globalThis.removeEventListener('offline', refreshInternetConnectedState);
    ...
    <>
      {portraitOrientationState ? (
        ...
        <div className="flex h-dvh w-screen flex-col items-center justify-center gap-2 text-zinc-400 select-none">
          <ArrowsIcon />
          <h1 className="text-center font-semibold">Draai je apparaat</h1>
        </div>
      )}
    </>
  );
